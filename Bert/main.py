...
# 合并成一个字符串，再按空格分隔
# 将上一步得到的单词列表转换为一个集合。集合是无序的，它会消除所有重复的单词，并且不会保留任何特定的顺序。
...
def make_data():
    ...
    while positive != batch_size/2 or negative != batch_size/2:
        ...
        for pos in cand_maked_pos[:n_pred]:
            ...
            elif random() > 0.9:  # 10% 随机替换
                index = randint(0, vocab_size - 1)
                while index < 4:# 确保不是特殊标记索引
                    index = randint(0, vocab_size - 1)
                input_ids[pos] = index
...
class Embedding(nn.Module):
    ...
    def forward(self, x, seg):
        ...
        pos = torch.arange(seq_len, dtype=torch.long) # 长度为seq_len的序列
        pos = pos.unsqueeze(0).expand_as(x)  # [seq_len]到[batch_size, seq_len]
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask):
        ...
        # 将 Q、K、V 映射到多头注意力的维度形式
        q_s = self.W_Q(Q).view(batch_size, -1, n_heads, d_k).transpose(1,2)
        # q_s: [batch_size, n_heads, seq_len, d_k]
        k_s = self.W_K(K).view(batch_size, -1, n_heads, d_k).transpose(1,2)
        # k_s: [batch_size, n_heads, seq_len, d_k]
        v_s = self.W_V(V).view(batch_size, -1, n_heads, d_v).transpose(1,2)
        # v_s: [batch_size, n_heads, seq_len, d_v]
